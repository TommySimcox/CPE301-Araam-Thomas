 ...
 * external sensors. 
 * -> This is an interrupt-based program. Interrupts are mainly used for 
 * updating sensor values. Sensors using interrupts to update sensor
 * data include the Water Level Module, Push Button, and Temperature/Humidity 
 * Sensor.
 ...
 *  water level threshold which determines when the program moves from Idle 
 ...
 * PROGRAM STATES
 * 
 * -> All States: The LCD is continuously refreshing every second with
 ...
 * with their own respective conditions. It is made sure that the 
 * corresponding LED is turned on and no vent activity 
 * is occurring.
 * -> Error State: Monitoring of water level is occurring 
 ...
 * with the only monitoring occurring on the Temperature/Humidity 
 * Sensor and Water Level sensor. Mainly the value temperature and
 * water_level is monitored until it is above or equal to 
 * temperature_threshold. Two state changes could occur at this
 * state being from Running State to Idle State or Running
 * State to Error State. This occurs then their conditons are met.
 ...
 * PROGRAM INTERRUPTS
 * 
 * -> INT3_vect: This interrupt is made to start an interrupt in the event
 * that the Push Button is pressed. During an external interrupt request 
 ...
 * -> ADC_vect: This interrupt is made to start an interrupt to the Stack
